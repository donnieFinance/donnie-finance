...
class ExchangeSwap {
    ...
    swapTokens(symbol1, symbol2, amountStr, amountOutMinStr){
        ...
        // amountOutMinStr 수량이 userReceives 이 양보다 어느정도 이상 작을 때 막기
        let amountOutMin = new Float64(amountOutMinStr);
        let errorRange = userReceives.multi(new Float64("0.89"));
        if(amountOutMin.lte(errorRange)) {
            throw new Error('Donnie: Insufficient amount');
        }
    ...
    addLiquidity(symbol1, symbol2, symbol1AmountStr, symbol2AmountStr){
        ...
        // symbol2InputAmount 이 값이 symbol2Amount  5%이상 크면 리턴.
        let errorRange = symbol2Amount.multi(new Float64("1.05"));
        if(symbol2InputAmount.gte(errorRange)) {
            throw new Error('Donnie: Insufficient Amount');
        }

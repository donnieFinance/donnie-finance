...
const AvaxDepositSwap = () => {
    ...
    */
    ...
    const [avaxGasPrice, setAvaxGasPrice] = useState(0);
    ...
    const gridOptions = {
        columnDefs: [
            {
                headerName: "No", field: "swapAvaxNo", width: 80
            },
            {
                headerName: "irc계정", field: "ircAccount", width: 200, cellRenderer: 'ircAccountRenderer'
            },
            {
                headerName: "swap계정(C-CHAIN)", field: "swapAccount", width: 200, cellRenderer: 'bepAccountRenderer'
            },
            {
                headerName: "입금AVAX", field: "avaxTokenAmount", width: 140
            },
            {
                headerName: "AvaxGas", field: "avaxGasWei", width: 120,
                valueGetter: function(params){
                    return params.data.avaxGasWei ? parseFloat(params.data.avaxGasWei, 'YYYY.MM.DD HH:mm') : null;
                }
            },
            {
            ...
            {
                headerName: "AVAX 잔고", field: "avaxTokenBalance",
    ...
    function sudongSendRenderer(props) {
        ...
        const onHandleClick = async() => {
            if(window.confirm(rowData.ircAccount+'의 '+ rowData.avaxTokenAmount +'토큰을 수동전송 하시겠습니까?')) {
                const params = {
                    iwSwapDepositNo: rowData.swapAvaxNo,
                    iwTokenName: iwTokenName,
                    ircAccount: rowData.ircAccount,
                    ercTokenAmount: rowData.avaxTokenAmount
                }
                //iwavax 수동 전송 기능
    ...
    function balanceCheckRenderer(props) {
        const rowData = props.data;
        const onHandleClick = async() => {
            let {data:avaxBalance} = await AdminApi.getCchainAVAXBalance(rowData.swapAccount);
    ...
    async function search() {
        setLoading(true);
        const {data} = await AdminApi.avaxDepositSwap();

        data.map(item => {
            item.avaxTokenAmount = parseFloat(item.avaxTokenAmount)
        })
        ...
        if(data) {
            data.map(item => {
                totalDeposit = totalDeposit + parseFloat(item.avaxTokenAmount);
                // console.log(item);
            })
    ...
    async function manualSendApprove(item) {
        ...
        if(window.confirm(item.ircAccount+'의 '+ item.avaxTokenAmount +'토큰을 avaxManager로 수동전송 하시겠습니까?')) {
            // 매니저  수동 전송
            const {data:approveResult} = await AdminApi.manualAvaxDeposit(item.swapAvaxNo);
    ...
    async function managerTransferOk(item) {
        if(window.confirm("상태값이 완료처리가 되어 배치처리가 됩니다!(수동전송이 아님! 주의! avaxscan에서 금액 확인후에 눌러주세요!)")) {
            const {data: result} = await AdminApi.updateAvaxSwapDepositFinished(item.swapAvaxNo);
            if (result === 200) {
                alert("변경이 완료되었습니다.");
                search();
            }
        ...
        <SearchBox>
            <Div>
                <Div>
                    ...
                    <Div ml={10}>AvaxManagerAccount : {managerAccount}</Div>
                    <Div ml={5}>
                        <Flex ml={10}>
                            <Div mr={10}>
                    ...
                    <Div ml={5}>
                        <Flex ml={10}>
                            ...
                            <Div ml={20}>
                                AvaxGasPrice : <Span fg="blue">{ComUtil.toCurrency(avaxGasPrice && avaxGasPrice.toFixed(2))}</Span>
                            </Div>
                        </Flex>
                    </Div>
                </Div>
...
export default AvaxDepositSwap;